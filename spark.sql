...
(https://spark.apache.org/docs/2.2.1/configuration.html#dynamic-allocation)
...
:help (to check in spark-shell)

spark.sql(""" Query """).coalesce(1).write.format("csv").option("header", "true").save("path")

path:"maprfs:/app/ASBI/scripts/sree/bkgs_0516_3"

:paste --> enter spark commands and then (ctrl-D to finish)
...
spark.mapred.max.split.size, 
spark.mapred.min.split.size
...
val spark = SparkSession
   ...
   jar and properties location we have to use spark shell
   
   /opt/mapr/spark/spark-2.2.1/bin/spark-submit --master yarn --deploy-mode cluster --files /opt/mapr/spark/spark-2.1.0/conf/hive-site.xml  --conf spark.local.dir=/opt/mapr/tmp/ --conf spark.shuffle.spill=true --conf spark.shuffle.service.enabled=true --conf spark.yarn.executor.memoryOverhead=600 --executor-memory 7g --driver-memory 6g --num-executors 40 --conf ""spark.driver.extraClassPath=/opt/mapr/hive/hive-2.1/lib/hive-hbase-handler-2.1.1-mapr-1703-r1.jar"" --conf ""spark.executor.extraClassPath=/opt/mapr/hive/hive-2.1/lib/hive-hbase-handler-2.1.1-mapr-1703-r1.jar"" --class com.cisco.services.asbi.financial.AS_Project_Tasks /hdfs/app/ASBI/scripts/jars/asbi_financial.jar /app/ASBI/param/asbi.properties as_project_tasks 2018-10-24 10:00:37
...
df_Project_Info_OP("PROJECT_ID").cast(LongType).alias("SK_AS_PROJECT_ID"),
   ...
   opt/mapr/spark/spark-2.2.1/bin/spark-submit --master yarn --deploy-mode cluster --files /opt/mapr/spark/spark-2.1.0/conf/hive-site.xml  --conf spark.local.dir=/opt/mapr/tmp/ --conf spark.shuffle.spill=true --conf spark.shuffle.service.enabled=true --conf spark.yarn.executor.memoryOverhead=600 --conf spark.network.timeout=1200s  --conf spark.sql.broadcastTimeout=1200 --executor-memory 7g --driver-memory 5g --num-executors 40 --properties-file /hdfs/app/ASBI/ETL/conf/asbi.properties --name as_proj_role_resrc_asgnmnt --class com.cisco.services.asbi.SparkHqlWrapper /hdfs/app/ASBI/scripts/jars/sparkhqlwrapper_2.11-0.8.jar /hdfs/app/ASBI/ETL/conf/asbi.properties /hdfs/app/ASBI/ETL/hql/as_proj_role_resrc_asgnmnt/as_proj_role_resrc_asgnmnt.csv as_proj_role_resrc_asgnmnt 2018-06-08 00:00:00.0
...
DESCRIBE extended dv_bdfrawzph_nogbd_r000_wh.CLM_ACES_PRFSNL_CLAIM_FACT PARTITION (load_ingstn_id=20190305)  (or )
DESCRIBE formatted dv_bdfrawzph_nogbd_r000_wh.CLM_ACES_PRFSNL_CLAIM_FACT PARTITION (load_ingstn_id=20190305) 
...
===============================
 ...
 URL: https://alvinhenrick.com/2017/05/16/apache-spark-analytical-window-functions/
...
useful links:
--------------------
https://stackoverflow.com/questions/41124428/spark-yarn-cluster-vs-client-how-to-choose-which-one-to-use/41142747
...
val df = spark.read.format("csv").option("header","true").option("infraSchema","true").load("hdfs://nameservicedvts2/dv/hdfsdata/ve2/bdf/rawz/phi/no_gbd/r000/warehouse/clm/aces/clm_aces_recovery_prof_clm_line")


val df = spark.read.parquet("hdfs://nameservicedvts2/dv/hdfsdata/ve2/bdf/srsz/phi/no_gbd/r000/warehouse/clm/aces/clm_aces_prfsnl_line_fact")
...
println("Checking Unix variables in scala shell")
...
spark2-submit --queue root.bdf.bdf_yarn --master yarn --deploy-mode cluster --conf spark.yarn.executor.memoryOverhead= --conf spark.sql.autoBroadcastJoinThreshold=-1 --num-executors 10 --executors-cores 2 --driver-memory 10g /${HADOOP_ENV}/app/ve2/bdf/rawz/phi/no_gbd/${RELEASE_VER}/etl/python/audit/bdf/file_based_load_validation.py ${HADOOP_ENV} /${HADOOP_ENV}/app/ve2/bdf/rawz/phi/no_gbd/${RELEASE_VER}/etl/parm/audit/bdf/bdf_file_validation.yaml

-------------
spark2-submit --queue root.bdf.bdf_yarn /${HADOOP_ENV}/app/ve2/bdf/rawz/phi/no_gbd/${RELEASE_VER}/etl/python/audit/bdf/file_based_load_validation.py ${HADOOP_ENV} /${HADOOP_ENV}/app/ve2/bdf/rawz/phi/no_gbd/${RELEASE_VER}/etl/parm/audit/bdf/bdf_file_validation.yaml

spark_submit = spark_submit+" "+queue_name+" --class"+" "+class_name+" "+jar_name+" "+date_format_pattern_hard_code+" "+sql_file_path+" "+status_file_path+" "+offset_path+" "+" "+hdfs_path+" "+app_name+" "+hbase_audit_table+" "+hbase_audit_bdf_load_log_table+" "+hbase_monitor_table+" "+hbase_summary_table+" "+hive_write_operation+" "+hive_append_operation+" "+database_name_sql_read
            #print(spark_submit)
...
from (
select tablename,load_status,count(*) as count from dv_bdfrawzph_nogbd_r000_qc.audit_bdf_load_validation group by tablename,load_status  order by tablename,load_status
) A)B
group by tablename """).show(100,False)